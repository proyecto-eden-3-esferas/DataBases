...
CREATE TABLE vernacular ( -- non-scientific names of plants, animals
  ...
  bname VARCHAR(45) NOT NULL,
  code CHAR(2)      DEFAULT 'en', -- the language code of the vernacular
...
CREATE TABLE crop_groups ( -- non-scientific names of plants, animals
  ...
  PRIMARY KEY  (crop),
  FOREIGN KEY (crop_group)  REFERENCES biology.life_types(life_type),
  FOREIGN KEY (code)        REFERENCES general.languages(code)
-- FOREIGN KEY (bname) REFERENCES plants(bname)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE pests ( -- pests or infectious diseases
  ...
  mechanism  VARCHAR(999), -- how the pest organism harms or parasitizes its host
  ...
  enemies    VARCHAR(99),  -- predators or antagonists of the pest
  PRIMARY KEY  (pest_id),
